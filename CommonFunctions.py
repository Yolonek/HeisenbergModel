from numpy import polyfit, linspace, poly1d, abs, zeros, vectorize, sum
import os
import json
from pyarma import trans, as_scalar
...
def check_if_file_has_data(file_path):
    if check_if_file_exists(file_path):
        if check_if_file_is_empty(file_path) is False:
            return True
        else:
...
def save_json_file(dict_with_data, json_file_name):
