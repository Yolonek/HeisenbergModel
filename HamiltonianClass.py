...
from pyarma import mat, eig_sym, norm
import numpy as np
...
class Hamiltonian(object):
    ...
    def print_hamiltonian_data(self):
        print(f'L = {self.L}, J = {self.J}, delta = {self.delta}')
    ...
    def calculate_ensemble(self, temperature):
        ...
        for eigenvalue in self.eigenvalues:
            exponent = 1 / exp(eigenvalue / temperature)
            eigenvalue_sum += exponent
    ...
    def calculate_mean_energy_power(self, temperature, power):
        ...
        for eigenvalue in self.eigenvalues:
            exponent = 1 / exp(eigenvalue / temperature)
            eigenvalue_sum += (eigenvalue ** power) * exponent
        return eigenvalue_sum / ensemble
    ...
    def calculate_specific_heat(self, temperature):
        mean_energy = self.calculate_mean_energy_power(temperature, 1)
        ...
        return round(specific_heat, 6)

    def calculate_specific_heat_range(self, temperature_range):
        temp_length = len(temperature_range)
        specific_heat_range = np.zeros(temp_length)
        for temperature, i in zip(temperature_range, range(temp_length)):
...
class QuantumState(Hamiltonian):
    ...
    def __init__(self, L, J, delta, is_pbc=False, is_reduced=False, vector_state=None):
        ...
        if vector_state is None:
            self.vector_state = self.get_nth_eigenvector(0)
        else:
    ...
    def calculate_variance(self):
        left_element = trans(self.vector_state)
        square_of_matrix = self.matrix * self.matrix
        expected_of_square = left_element * (square_of_matrix * self.vector_state)
    ...
    def set_vector_from_eigenstate(self, eigenvector_number):
        self.vector_state = self.get_nth_eigenvector(eigenvector_number)

    def set_state_vector(self, pyarma_vec):
        self.vector_state = pyarma_vec

    def print_vector_state(self):
        vector_to_print = f''
        for basis_element, index in zip(self.basis, range(self.size)):
            vector_value = round(self.vector_state[index], 4)
            if vector_value != 0:
    ...
    def set_basis_element_to_state_vector(self, basis_element):
        if basis_element in self.basis:
            ...
            self.vector_state = basis_vector
        else:
    ...
    def find_temperature_for_state_energy(self, temperature_range):
        ...
        stop_time = time()
        print(f'Temperature corresponfing to energy {energy_of_state} is: {found_temperature},'
    ...
    def coefficient_c_of_t(self, time_value, eigenstate_number):
        ...
        product = trans(nth_eigenvector) * self.vector_state
    ...
    def set_spin_operator(self, spin_number, assign=True):
        ...
        for basis_state, index in zip(self.basis, range(self.size)):
    ...
    def calculate_operator_time_evolution(self, time_range):
        ...
        for dt, index in zip(time_range, range(list_length)):
    ...
    def lanczos_step(self):
        if self.lanczos_matrix is None and self.lanczos_steps == 0:
            ...
            self.lanczos_vector = self.vector_state
        else:
