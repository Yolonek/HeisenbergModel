...
class EnergyGap(object):
    ...
    def simulate_energy_gap(self):
        for delta in self.delta_list:
            ...
            energy_delta_range = []
            for L, index in zip(self.L_list, range(len(self.L_list))):
                start_time_sim = time()
                ...
                stop_time_sim = time()
                print(f'Delta = {delta}, L = {L}, Energy delta = {energy_delta} seconds, '
                      f'time elapsed: {round(stop_time_sim - start_time_sim, 3)}')
    ...
    def plot_energy_gap(self):
        ...
        for delta in self.delta_list:
            ...
            axis[index].plot(extrapolated_x, extrapolated_y, label=polynomial)
            axis[index].scatter(inverse_of_L, energy_gaps, color='black')
            axis[index].set_title(f'Delta: {delta}')
            axis[index].set(ylim=(0, max(energy_gaps) + 0.1))
            axis[index].grid()
            axis[index].legend(loc='upper left')
        axis[0].set(ylabel='Energy gap')
